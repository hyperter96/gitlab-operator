...
def gitlab_registry
  ENV['GITLAB_REGISTRY_BASE_URL'] || File.dirname(ENV['CI_REGISTRY_IMAGE'] ||
      'registry.gitlab.com/gitlab-org/cloud-native/gitlab-operator')
end
...
version = ARGV[0]
puts "Using #{version} as the tag to submit for certification"
...
project_data.each_key do |name|
  ...
  if ENV['CI_COMMIT_REF_NAME'] == 'master' || regular_tag?
    version = "#{ENV[project_data[name]['version_variable']].sub(/-(ce|ee)$/, '')}"
  end
  ...
  if project_data.key? name
    ...
    errors << "#{name}: No project info listed in gitlab-projects.yaml"
  end
end
