...
func (r *ReconcileGitlab) reconcileTaskRunnerDeployment(cr *gitlabv1beta1.Gitlab) error {
	tasker := getTaskRunnerDeployment(cr)

	if gitlabutils.IsObjectFound(r.client, types.NamespacedName{Namespace: cr.Namespace, Name: tasker.Name}, tasker) {
		return nil
	}
	...
	return r.client.Create(context.TODO(), tasker)
}
